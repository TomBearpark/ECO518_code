...
pacman::p_load(ggplot2, dplyr, sf, tigris, 
               viridis, patchwork, sandwich, nlme, car, jtools)
theme_set(theme_bw())
# Set paths
...
variable. Color by average income, just to see if that seems like an
...
ggplot(df) + 
  geom_point(aes(x = sqr_u2, y = enrl_tot, color = log(avginc)), alpha= .7) + 
  scale_color_viridis()
```
...
```{r}
df$inv_resid_weight_lm2 <- 1 / df$u2
```
...
Create a plot - with regression coefficients and the SE, both raw, and
clustered, for each model.
...
effects, not just the significancelevels of tests, should inform your
views on this.
