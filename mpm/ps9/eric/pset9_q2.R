...
num_choices <- 5
names(df) <- 
  c(paste0("ic_", seq(1,num_choices)), paste0("oc_", seq(1,num_choices)), "choice", "i")
N <- dim(df)[1]
...
# Add columns for the constants - don't give one to the first one 
nC <- num_choices - 1
...
for (i in 1:N ){
  ...
  Delf <- matrix(c(alpha_i, beta_i))
  Omega <- Omega + Delf %*% t(Delf)
}
