...
even if we want to get a credible set rather than a confidence interval. 
...
plot, the blue lines represent the bootstrapped CI found in part c above. The red lines
...
ggplot(plot_df) + 
  ...
  geom_vline(xintercept = CI, color = "blue", alpha = 0.5)
```

We can see that the credible set found in part 1d) is much tighter than that 
found when we used the bootstrap estimate. 
