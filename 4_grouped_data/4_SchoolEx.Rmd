...
```{r}
plot_summs(lm2, lm4, scale = TRUE, model.names = c("Unweighted", "Weighted"))
```
...
```{r}
library(caret)
featurePlot(x = df[vars], y = df$testscr)
```
...
X <- model.matrix(formula(paste0(reg2, "+ log(avginc)")), df)
